...
efficient execution plan over the [`pto-einsum`](../pto-einsum) library.

This document replaces the stale `PLANNER_DESIGN.md` that used to live in
`pto-einsum`. It states the thesis, the IR, the planner features, the codegen
targets, the correctness discipline, and the build order, with the full
...
**realized state** and is kept in sync with it. All of the features below are built:
the IR + staged executor + correctness gates, the read-mode / fused-output planner,
graph capture (the staged chain replayed as one dispatch, bit-exact, with the
launch-bound dispatch-elimination win measured), and the fused-node backend that
hosts the resident-state scan and the gated-kkt kernels as capturable single-dispatch
nodes — with the staged-vs-fused decision keeping each only where it gates
bit-faithful + deterministic AND beats staged-captured (scan 4.4× / 2.3×, kkt 2.0×).
Any divergence — notably that read-mode / fused-output selection is realized *inside*
the soft-frozen library and the fuser *selects among* those modes, and that the
fused-node backend *hosts* the proven kernels rather than templating epilogue codegen
(the single-`.so` opaque inline of §9 is left as unproven research) — is called out in
that document's sync ledger.
...
1. **Fusion is a separate layer on top of `einsum()`, not baked into it.** The
   library compiles *one* contraction at a time and is soft-frozen. Everything
...
2. **The whole chunk-attention taxonomy decomposes into library primitives plus
   a small set of opaque nodes.** Validated end-to-end:
   - **T0** (`kkt`): matmul-core + on-chip gated epilogue (`src/pto_fuser/kernels/` (kkt_fused)).
   - **T2** (`chunk_h`): sequential cross-chunk recurrence with resident state that
     is *both* a matmul operand and a Vec accumulator (`src/pto_fuser/kernels/` (chunk_h_scan)).
   - **T3** (DeltaNet/WY): an opaque triangular-inverse node bracketed by einsum
     ...
     (the DeltaNet forward in `pto_fuser.forwards`). The full 6-stage forward is bit-faithful to an
...
3. **Fusion buys dispatch-elimination, not compute.** Measured repeatedly: at the
   ...
   planner's first-class feature is *dispatch-elim via graph capture*, and monolithic
...
that (a) picks the cheapest library read/store mode per stage, (b) fuses
...
**Out of scope (lives in the library, soft-frozen):** the contraction codegen
...
them. If the fuser needs a library capability that does not exist, that is a
demand-driven library change (see `pto-einsum` roadmap), made deliberately and
...
The core unit. Wraps one library `einsum()` call plus the library annotations
...
Lowers to the library's Vec kernels (elementwise Hadamard, broadcast/scaling) or,
...
Ordered by the evidence for their payoff. Each feature is *opt-in per node* and must
pass the correctness gate (§6) against the default lowering before it is kept.

1. **Graph capture (dispatch-elim) — the highest-payoff feature.** NPUGraph captures the PTO
...
2. **Read-mode selection (§2.11–2.13) — eliminates Phase A.** Pick NT / NN-strided
   ...
   Phase-A path, on the GDN shapes. Pure library selection; the planner reads the
...
5. **Resident-state scheduling.** For sequential recurrences, schedule
   ...
   library primitives (in_nt=2 + in_nt=3) are unchanged.
...
**Feature ordering rule:** prefer (1)+(2)+(3)+(4) — staged stages with the right
...
- **Staged launch (baseline).** Each node → its own library `.so` (persistent
...
- **Fused-node.** A generated kernel = matmul-core + pluggable load front-end +
  pluggable Vec epilogue/prologue (glue absorption), optionally inlining an opaque
  AICORE device-fn (monolithic fusion). This is the only backend that emits *new*
  device code; it
  reuses the library's tile-matmul core as a header-level building block, it does
  not fork it.

The library stays the single source of the matmul/Vec/transpose codegen. The
...
The gate is part of the planner loop, not a separate test phase: a feature that does
...
The chunked DeltaNet forward (`pto_fuser.forwards.deltanet`) is the reference the design is calibrated
to. The 6-stage forward, as IR:

| # | stage            | node                                    | features in play |
|---|------------------|-----------------------------------------|----------------|
| 1 | `cumsum(g)`      | `VecGlueNode` (scan)                     | scan edge invariant; absorb→2 |
| 2 | `kkt → A`        | `EinsumNode` "nid,njd->nij" + tril epi   | NT read, gated epilogue |
...
| 5 | `chunk_delta_h`  | `EinsumNode` scan (resident S)           | TN + NN-strided; resident-state |
| 6 | `chunk_o → o`    | `EinsumNode` + causal mask + scale       | NT read; gated epilogue |

Every row is a proven primitive class. The planner's job on this graph: pick read
modes (col. "features"), absorb stages 1/2/6 glue into adjacent contractions, host
...
already bit-faithful (validated); each feature is then gated in.

---

## 8. Build status

The features are delivered in payoff order, each behind the §6 gate. See
[`IMPLEMENTATION.md`](IMPLEMENTATION.md) for the realized state, measurements, and
the design↔implementation sync ledger.

- **IR + staged executor + gate.** The three node types, a staged backend that runs
  the DeltaNet forward from an IR description, and the frob_rel + determinism gate
  harness. Validated: DeltaNet forward from IR, staged, gated green at M=16384.
- **Read-mode / fused-output planner.** Read-mode selection, fused-output store, and
  glue-absorption *detection* as gated annotations, plus the opaque-node registry
  (tri_inv). Each feature is measured on the GDN/DeltaNet stages and kept only where
  gated-green and faster than the default.
- **Graph capture.** Capture the staged chain, replay as one dispatch — the
  dispatch-elimination win at small `T` (~2.4× at T=128), perf-neutral and still
  bit-exact at T≥512.
- **Fused-node backend + fusion decision.** Resident-state scheduling and glue
  absorption as hosted fused kernels, *only* for the regimes graph capture leaves on
  the table, each behind a measured launch-bound justification and a determinism
  gate: a documented decision per stage — staged-captured vs fused — with the
  measurement that chose it.

A second reference forward (GDN or KDA, which share the kkt/wy/chunk_h/chunk_o
stages) confirms the planner generalizes beyond the one worked example.
...
- **Static-shape assumption for graph capture.** Replay needs shapes fixed across
  ...
  small family of captures keyed by shape bucket. Quantify before committing to graph capture.
- **Opaque-node inlining ABI.** Lever 6 inlines an AICORE device-fn through GM; the
  ...
  library's (`--npu-arch=dav-2201`, same a2/a3 family) inside *one* `.so` is
  unproven — staged multi-`.so` sharing GM works today, single-`.so` inline does
  not yet. Treat as research, not assumed.
- **Library gaps surfaced by the planner** are logged to the `pto-einsum`
